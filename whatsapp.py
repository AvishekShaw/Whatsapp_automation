...
for name in names.keys():
    ...
    if send_message:
        ...
        attachment_icon_xpath = '//span[@data-icon="attach-menu-plus"]'
        attachment_icon = driver.find_element(By.XPATH, attachment_icon_xpath)
        attachment_icon.click()
        time.sleep(1)

        # find attach-document icon xpath and send doc
        attach_doc_xpath = '//span[text()="Document"]/input'
        attach_doc_icon = driver.find_element(By.XPATH, attach_doc_xpath)
        attach_doc_icon.send_keys(
            "/home/avishek/Code/data/whatsapp_automation/Invitation.pdf"
        )
        time.sleep(3)
